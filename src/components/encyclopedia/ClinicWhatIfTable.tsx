...
import { Users, TrendingUp, Calculator, DollarSign, Edit3, MapPin, Globe, Building2, Download, Share2, Save, FolderOpen, GitCompare, Trash2 } from 'lucide-react';
...
export function ClinicWhatIfTable() {
  ...
  const saveScenario = async () => {
    ...
    const { error } = await supabase.from('roi_scenarios').insert([{
      ...
      configuration: inputs as unknown as Record<string, unknown>,
              ...
              <div className="space-y-2 py-4 max-h-[60vh] overflow-y-auto">
                {savedScenarios.length === 0 ? (
                  ...
                  savedScenarios.map(scenario => (
                    <div key={scenario.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-muted/50">
                      <div>
