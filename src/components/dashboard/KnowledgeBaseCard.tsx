...
import { Database, ArrowLeft, Package } from 'lucide-react';
...
export function KnowledgeBaseCard({ animationDelay = 0, activeAssets = 0 }: KnowledgeBaseCardProps) {
  return (
    <div className="h-full flex flex-col gap-2">
      <Link to="/knowledge-registry" className="block flex-1">
        <Card 
      ...
      <Link to="/module-downloads" className="block">
        <Card className="border-purple-500/30 bg-gradient-to-r from-purple-500/5 to-indigo-500/5 hover:border-purple-500/50 hover:shadow-lg transition-all duration-300 cursor-pointer group p-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium flex items-center gap-2">
                Module Downloads
                <ArrowLeft className="h-3 w-3 opacity-0 group-hover:opacity-100 transition-opacity text-purple-500" />
              </p>
              <p className="text-[10px] text-muted-foreground">הורדת מודולים כקבצי ZIP</p>
            </div>
          </div>
        </Card>
      </Link>
